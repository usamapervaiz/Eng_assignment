...
def get_args():
    ...
    parser.add_argument('--data_path', type=str, default='/Users/upervaiz/Desktop/challenge/lines_286.txt')
    
    parser.add_argument('--bentley_ottmann', type=bool, default=False)
...
if __name__ == '__main__':
    ...
    lines, number_of_lines = load_data(args.data_path)
    # Plot the Input data (and convert to list format)
    lines_segs =  plot_data(lines, plot = False)
    
    # Run the line-line intersection algorithm
    start_time = time.time()
    
    if not (args.bentley_ottmann):
